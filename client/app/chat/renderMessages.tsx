"use client"
import { Message, RenderMessageProps } from "./interface"
import { useCreateSocketConnection } from "./socketHandler"

export const RenderMessages: React.FC<RenderMessageProps> = ({
  bottomRef,
  conversation,
  socketId
}) => {
    ...
    const getConversation = () => {
      ...
      return conversation.map((msg: Message, index: number) => {
          ...
          <div key={index} className={containerClass}>
            <div className={messageClass}>
              ...
              {msg.type === "emoji" ? (
                ...
                <span className="inline-block mr-2">{msg.text}</span>
              )}
            </div>
          </div>
        )
...
RenderMessages.displayName = "RenderMessages" // Set display name for debugging purposes
